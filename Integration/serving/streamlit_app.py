...
def _units_sold_expr() -> str:
    return "COALESCE(quantity, `product`.quantity, 1)"


def _unit_price_expr() -> str:
    return "COALESCE(price, `product`.price, 0.0)"


def _total_sales_expr() -> str:
    return f"({_units_sold_expr()} * {_unit_price_expr()})"
...
def batch_kpis_from_df(df: pd.DataFrame) -> dict:
    ...
    numeric_cols = [
        ...
        if c.lower() in (
            "cnt", "count", "counts", "events", "orders",
            "revenue_usd", "total_sales_usd", "units_sold"
        )
    ]
...
def sql_overview(t: str, start_sql: str, end_sql: str) -> str:
    ...
    units = _units_sold_expr()
    total_sales = _total_sales_expr()
...
SELECT
  ...
  SUM(CASE WHEN event_name = 'purchase' THEN {units} ELSE 0 END) AS units_sold,
  SUM(CASE WHEN event_name = 'purchase' THEN {amt} ELSE 0.0 END) AS realized_revenue_usd,
  SUM(CASE WHEN event_name = 'purchase' THEN {total_sales} ELSE 0.0 END) AS total_sales_usd,
...
WHERE {where}{extra}
GROUP BY {expr}
ORDER BY total_sales_usd DESC, revenue_usd DESC, orders DESC, events DESC
"""
...
WHERE {where}
  AND {expr} <> 'unknown'
GROUP BY {expr}
ORDER BY total_sales_usd DESC, revenue_usd DESC, orders DESC, product_views DESC
"""
...
with st.sidebar:
    ...
    st.caption(
        ...
        "Total sales se calcula como quantity * price. "
...
with tab_hist:
    ...
    st.caption(
        "Aquí sí se aprovecha el esquema rico: revenue, order, marketing, customer, device, geo, product, quantity y price."
    )
        ...
        if run:
            try:
                ...
                if not df.empty:
                    ...
                    total_sales = float(row.get("total_sales_usd", 0.0) or 0.0)
                    ...
                    c8.metric("Unidades vendidas", _fmt_int(units_sold))

                    c9, c10, c11, c12 = st.columns(4)
                    c9.metric("Revenue", _fmt_money(revenue))
                    c10.metric("Total sales", _fmt_money(total_sales))
                    c11.metric("View → Cart", _fmt_pct(_ratio(carts, views)))
                    c12.metric("AOV", _fmt_money(aov))

                    c13, c14 = st.columns(2)
                    c13.metric("Cart → Checkout", _fmt_pct(_ratio(checkouts, carts)))
                    c14.metric("Checkout → Purchase", _fmt_pct(_ratio(purchases, checkouts)))
            ...
            if run:
                try:
                    ...
                    if not df.empty and "time_bucket" in df.columns:
                        ...
                        total_sales = float(pd.to_numeric(df.get("total_sales_usd", 0), errors="coerce").fillna(0).sum())
                        total_units = int(pd.to_numeric(df.get("units_sold", 0), errors="coerce").fillna(0).sum())
                        ...
                        avg_daily_sales = total_sales / len(df) if len(df) else 0.0

                        r1, r2, r3, r4 = st.columns(4)
                        r1.metric("Dias", _fmt_int(len(df)))
                        r2.metric("Revenue total", _fmt_money(total_rev))
                        r3.metric("Total sales", _fmt_money(total_sales))
                        r4.metric("Unidades vendidas", _fmt_int(total_units))

                        r5, r6, r7 = st.columns(3)
                        r5.metric("Órdenes", _fmt_int(total_orders))
                        r6.metric("Revenue promedio / día", _fmt_money(avg_daily_rev))
                        r7.metric("Sales promedio / día", _fmt_money(avg_daily_sales))

                        if "total_sales_usd" in df.columns:
                            line_chart(df, x="time_bucket", y="total_sales_usd", title="Total sales por día", height=250)
    ...
    with hist_tabs[2]:
        ...
        with c1:
            ...
            metric_marketing = st.selectbox(
                "Métrica marketing",
                ["total_sales_usd", "revenue_usd", "units_sold", "orders", "sessions", "users", "events"],
                key="marketing_metric"
            )
        ...
        with c2:
            ...
            metric_customer = st.selectbox(
                "Métrica cliente",
                ["total_sales_usd", "revenue_usd", "units_sold", "orders", "buyers", "users", "events"],
                key="customer_metric"
            )
    ...
    with hist_tabs[3]:
        ...
        metric_product = st.selectbox(
            "Métrica producto",
            ["total_sales_usd", "revenue_usd", "units_sold", "orders", "buyers", "purchase_events", "begin_checkout", "add_to_cart", "product_views"],
            key="product_metric",
        )
        ...
        if run:
            try:
                ...
                if {"dimension", metric_product}.issubset(df.columns):
                    ...
                    total_units = int(pd.to_numeric(df2.get("units_sold", 0), errors="coerce").fillna(0).sum())
                    total_rev = float(pd.to_numeric(df2.get("revenue_usd", 0), errors="coerce").fillna(0).sum())
                    total_sales = float(pd.to_numeric(df2.get("total_sales_usd", 0), errors="coerce").fillna(0).sum())
                    ...
                    c4.metric("Unidades vendidas", _fmt_int(total_units))

                    c5, c6 = st.columns(2)
                    c5.metric("Revenue", _fmt_money(total_rev))
                    c6.metric("Total sales", _fmt_money(total_sales))
    ...
    with hist_tabs[4]:
        ...
        with top_l:
            ...
            metric_ops = st.selectbox(
                "Métrica operativa",
                ["total_sales_usd", "revenue_usd", "units_sold", "orders", "users", "sessions", "events"],
                key="ops_metric"
            )
